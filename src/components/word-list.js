const WordList = [
  {
    id: 0,
    name: "Reading Word List",
    words: [
      {
        id: 0,
        word: "Concubine",
        definition:
          "A woman who lives with a man but has lower status than his wife or wives",
        complete: false,
      },
      {
        id: 1,
        word: "Tacit",
        definition: "Understood or implied without being stated.",
        complete: false,
      },
      {
        id: 2,
        word: "Eponymous",
        definition:
          "The person who's name is given for something.  (Moore's Law)",
        complete: false,
      },
      {
        id: 3,
        word: "Polemic",
        definition: "A strong verbal or written attack on someone or something",
        complete: false,
      },
      {
        id: 4,
        word: "Pecuniary",
        definition: "Relating to money",
        complete: false,
      },
      {
        id: 5,
        word: "Ambrosia",
        definition: "Food or drink conferring immortality",
        complete: false,
      },
      {
        id: 6,
        word: "Taciturn",
        definition: "Silent, or disinclined to talk",
        complete: false,
      },
      {
        id: 7,
        word: "Laconic",
        definition: "Saying as little as possible, pithy",
        complete: false,
      },
      {
        id: 8,
        word: "Loquacious",
        definition: "Very talkative",
        complete: false,
      },
      {
        id: 9,
        word: "Assiduous",
        definition: "Having great care and perseverance",
        complete: false,
      },
      {
        id: 10,
        word: "Incorrigible",
        definition:
          "Not being able to be corrected, improved, or reformed. Synonyms:	hardened, dyed-in-the-wool, incurable, chronic, irredeemable, hopeless.",
        complete: false,
      },
      {
        id: 11,
        word: "Sinecure",
        definition:
          "An office that requires or involves little or no responsibility, labour, or active service.",
        complete: false,
      },
      {
        id: 12,
        word: "Specious",
        definition: "Superficially plausible, but actually wrong",
        complete: false,
      },
      {
        id: 13,
        word: "Peripéteia",
        definition: "A turning point in a story",
        complete: false,
      },
      {
        id: 14,
        word: "Abattoir",
        definition: "A slaughterhouse",
        complete: false,
      },
      {
        id: 15,
        word: "Egress",
        definition: "The action of going out or leaving",
        complete: false,
      },
      {
        id: 16,
        word: "Mercurial",
        definition:
          "Subject to sudden or unpredictable changes of mood or mind",
        complete: false,
      },
      {
        id: 17,
        word: "Chimera",
        definition:
          "(Ki mirror a) A fire-breathing female monster. Mythical creature",
        complete: false,
      },
      {
        id: 18,
        word: "Sardonic",
        definition: "Grimly mocking or cynical",
        complete: false,
      },
      {
        id: 19,
        word: "Epistemic",
        definition:
          "Of or relating to theory of knowledge, the study of knowledge and justified belief",
        complete: false,
      },
      {
        id: 20,
        word: "Nihilism",
        definition:
          "The rejection of all religious and moral principles, often in the belief that life is meaningless.",
        complete: false,
      },
      {
        id: 21,
        word: "Omakase",
        definition:
          "A Japanese meal consisting of dishes selected by the chef.",
        complete: false,
      },
      {
        id: 22,
        word: "Somatic",
        definition:
          "Of or relating to the body, especially as distinct from the mind.",
        complete: false,
      },
    ],
  },
]

export default WordList
